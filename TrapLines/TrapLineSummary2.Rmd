...
```{r readData}
# Read the data
beginAt <- 22 # day in month at which to begin recording periods. Good options are 10/11 and 22/23
tl <- getTrapNZData()
yr <- tl$yr
tl <- tl$tl
...
nBox <- with(tl[-grep(' trap2', tl$code),], table(line,
                                gsub('([[:upper:]]+)([[:digit:]]+)', '\\2', code)))
...
tt <- catch[, colnames(catch) %in% c('State Highway','Lockingtons','Marble Hill',
                                     'Maruia River','Kellys','Campground')]
ttPY <- catchPY[, colnames(catchPY) %in% c('State Highway','Lockingtons','Marble Hill',
                                           'Maruia River','Kellys','Campground')]
...
Finally, in the year to date, the following non-target species have been caught:
 `r mSppFn(miscSppY, 'Weka', 'weka')` and
  `r mSppFn(miscSppY, 'Bird', 'other birds')`.

```{r OtherSeeComment}
osc <- tl[tl$species.caught %in% c('Other','Unspecified'),]
osc <- osc[osc$notes!='',]
```
`r if(nrow(osc)>0) paste(c('Other non-target species caught in the year to date are', osc$notes), collapse=', ')`.
\color{black!50}

### Check

Here are the contents of miscellaneous species files for the year-to-date and current month (\texttt{miscSppY} and \texttt{miscSppM}, respectively) as a check that nothing has been missed from the two previous sub-sections:
  
```{r miscSppChk}
chkTab <- merge(cbind(Spp=names(miscSppY), ToDate=miscSppY),
                cbind(Spp=names(miscSppM), Month=miscSppM), all=TRUE)
excl <- c('N/A','None','Rat','Stoat','Weasel')
...
# To load a new coordinates file:
  # * save the GPX file
  # * use an online converter to change it to a csv file
  # * edit in LibreOffice
  # * then read into R.

# Rename code to match Waypoints
...
```{r, mthTrapCatch1, fig.height=10, fig.width=8, fig.cap="\\label{mthTrapCatch1}Map of the Lake Daniell lines (i.e., Lake Daniell Track and Terrace, Alfred 4x4, Pell Terrace and Reid Stream).  Each line is a different colour.  The point diameters are proportional to the cumulative catch, and the colour indicates when the mammal was caught (months prior to the present).  The grey + signs indicate the trap locations (necessary for traps which have caught nothing in the past 12 months).  The topographical map overlay is approximate only.  Note, the GPS locations of traps 46-48 on Reid Stream are not current, and the GPS trap numbers on Alfred 4x4 are slightly out of synchrony with the actual trap numbers."}
par(mar=rep(0, 4))
...
{  
  ...
  with(ttLines, plot(E, S, pch=3, cex=0.7, col='grey50', axes=FALSE, xlab='', ylab=''))
  ttUsr <- par()$usr
  rasterImage(myMap, ttUsr[1], ttUsr[3]-0.002, ttUsr[2], ttUsr[4]+0.0005)
  with(ttLines, points(E, S, pch=3, cex=0.7, col='grey50'))
...
```{r, mthTrapCatch2, fig.height=10, fig.width=8, fig.cap="\\label{mthTrapCatch2}Map of the Lewis Pass H/Way and Campground lines.  Each line is a different colour.  The point diameters are proportional to the cumulative catch, and the colour indicates when the mammal was caught (months prior to the present).  The grey + signs indicate the trap locations (necessary for traps which have caught nothing in the past 12 months). The topographical map overlay is approximate only."}
par(mar=rep(0, 4))
...
{  
  ttLines <- catch12m[gsub('[[:digit:]]', '', catch12m$Trap) %in% lineGrps[[lns]],]
  ttCum <- cumCatch[gsub('[[:digit:]]', '', cumCatch$Trap) %in% lineGrps[[lns]],]
  with(ttLines, plot(E, S, pch=3, cex=0.7, col='grey50', axes=FALSE, xlab='', ylab='', type='n'))
  ttUsr <- par()$usr
