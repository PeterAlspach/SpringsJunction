...
```{r, nonTarget}
nonTar <- table(unlist(tl[, c('Trap.1','Trap.2')]))
```

Finally, in the year to date, the following non-target species have been caught: 
`r if ('Robin' %in% names(nonTar)) nonTar[names(nonTar)=='Robin'] else 0` robin,
`r if ('Weka' %in% names(nonTar)) nonTar[names(nonTar)=='Weka'] else 0` weka, and 
`r if ('Harrier' %in% names(nonTar)) nonTar[names(nonTar)=='Harrier'] else 0` harrier.
