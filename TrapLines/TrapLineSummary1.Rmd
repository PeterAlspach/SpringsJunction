...
# Omit the following two lines if the first report for the year 
tlXtra <- read.csv('TrapCatchesRecent.csv', header=TRUE, sep=',', stringsAsFactors=FALSE)
tl <- rbind(tl, tlXtra)
tl <- tl[tl$Line!='Possum A12',]

# Omit following line unless final report for the month
# write.csv(tl, 'TrapCatches.csv', row.names=FALSE) 
...
cM1 <- currMth[!(currMth$Status %in% c('Missing', 'Maintenance required not functioning')),]
nTrap1 <- table(cM1$Line, gsub('([[:upper:]]+)([[:digit:]]+)', '\\2', cM1$Tag.No))
nTrap1 <- apply(nTrap1, 1, function(x) length(x[x!=0]))

catch <- merge(nTrap1, catch, by='row.names', all=TRUE)
...
Table \ref{currMthN} (below) summarises the number of mammals caught in the most recent monthly interval for each line.  The first column is the number of trap boxes^[In some cases trap boxes may be missing or not functioning, and these have been omitted.  Hence the numbers here may not correspond to totals elsewhere], followed by the number of traps which were sprung but empty.  The remaining columns (in blue) give the catch of rats, stoats, weasels, other mammals (e.g., cats, ferrets, rabbits) and the total mammals.  
