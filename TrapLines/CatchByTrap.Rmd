...
ttTab <- with(tlAll[tlAll$Line=='Lake Daniell Terrace',], table(Trap.No, Trap.1))
rownames(ttTab[apply(ttTab[, c("Still Set","Still Set Bait Eaten")], 1, sum, na.rm=TRUE)==
                 apply(ttTab, 1, sum, na.rm=TRUE),])
tlAll[regexpr('(Needs)|(rusty)|(broken)', tlAll$Comment)>0 & regexpr('Lake', tlAll$Line)>0,
      c('Line','Trap.No','Comment')]
with(tlAll[regexpr('(Needs)|(rusty)|(broken)', tlAll$Comment)>0,], table(Comment, Trap.No, Line))
```
